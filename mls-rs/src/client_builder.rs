...
impl<C: IntoConfig> ClientBuilder<C> {
    ...
    /// Set the lifetime duration of key packages generated by the client.
    pub fn key_package_lifetime(self, lifetime: Duration) -> ClientBuilder<IntoConfigOutput<C>> {
        let mut c = self.0.into_config();
        c.0.settings.lifetime = lifetime;
        ClientBuilder(c)
    }
    ...
    pub(crate) fn key_package_not_before(
        self,
        key_package_not_before: MlsTime,
    ) -> ClientBuilder<IntoConfigOutput<C>> {
        let mut c = self.0.into_config();
    ...
    Cp: CryptoProvider + Clone,
{
    ...
    fn lifetime(&self, timestamp: Option<MlsTime>) -> Lifetime {
        ...
        Lifetime {
            ...
            not_after: now_timestamp + self.settings.lifetime,
        }
    }
...
pub(crate) struct Settings {
    ...
    #[cfg(any(test, feature = "test_util"))]
    pub(crate) key_package_not_before: Option<MlsTime>,
}
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ...
            lifetime: 365 * 24 * Duration::from_secs(3600),
