...
pub enum MlsError {
    ...
    #[cfg_attr(feature = "std", error("Invalid application data proposal"))]
    InvalidApplicationDataProposal,
    #[cfg_attr(feature = "std", error("Invalid application data update proposal"))]
    InvalidApplicationDataUpdateProposal,
    ...
    C: ClientConfig + Clone,
{
    ...
    pub async fn generate_key_package_messages(
        ...
        for _ in 0..count {
            let kp_gen = self
                ...
                .await?;
            let (id, kp_data) = kp_gen.to_storage()?;
            kps_data.insert(id, kp_data);
            kps.push(kp_gen);
        }
        self.config
            .key_package_repo()
    ...
    pub async fn external_add_proposal(
        ...
        self.external_add_proposal_with(
            group_info,
            tree_data,
            authenticated_data,
            key_package_extensions,
...
pub(crate) mod test_utils {
        ...
        F: FnMut(&mut TestClientConfig),
    {
        ...
        let mut client = TestClientBuilder::new_for_test()
            ...
            .signing_identity(identity.clone(), secret_key)
            .build();
...
mod tests {
    ...
    async fn new_member_add_proposal_adds_to_group() {
        ...
        let bob = TestClientBuilder::new_for_test()
            ...
            .signing_identity(bob_identity.clone(), secret_key)
            .build();
    ...
    async fn join_via_external_commit(do_remove: bool, with_psk: bool) -> Result<(), MlsError> {
        ...
        let new_client = TestClientBuilder::new_for_test()
            ...
            .ciphersuite(TEST_CIPHER_SUITE)
            .signing_identity(new_client_identity.clone(), secret_key)
            .build();
    ...
    async fn creating_an_external_commit_requires_a_group_info_message() {
        ...
        let alice = TestClientBuilder::new_for_test()
            .ciphersuite(TEST_CIPHER_SUITE)
            .signing_identity(alice_identity.clone(), secret_key)
            .build();
    ...
    async fn external_commit_with_invalid_group_info_fails() {
        ...
        let carol = TestClientBuilder::new_for_test()
            .ciphersuite(TEST_CIPHER_SUITE)
            .signing_identity(carol_identity, secret_key)
            .build();
