...
use crate::group::{
    proposal::AppDataUpdateOperation, proposal::AppDataUpdateProposal, ComponentId,
};
...
impl<C: ExternalClientConfig + Clone> ExternalGroup<C> {
    ...
    /// Create an external proposal to update an application component
    ///
    /// # Warning
    ///
    /// In order for the proposal generated by this function to be successfully
    ...
    #[cfg(all(feature = "by_ref_proposal", feature = "application_data"))]
    #[cfg_attr(not(mls_build_async), maybe_async::must_be_sync)]
    pub async fn propose_application_data_update(
        &mut self,
        component_id: ComponentId,
        op: AppDataUpdateOperation,
        authenticated_data: Vec<u8>,
    ) -> Result<MlsMessage, MlsError> {
        let proposal = Proposal::AppDataUpdate(AppDataUpdateProposal { component_id, op });
