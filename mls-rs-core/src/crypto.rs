...
/// A pre-shared key and its identifier for use with PSK-mode HPKE
...
pub trait CipherSuiteProvider: Send + Sync {
    ...
    /// Encrypt the plaintext `pt` to the public key `remote_key` using PSK mode HPKE
    /// (RFC 9180). The [`HpkePsk`] binds the ciphertext to knowledge of the pre-shared
    /// key. The PSK value must be at least 32 bytes.
    ...
    /// Decrypt the `ciphertext` generated by [`hpke_seal_psk`](CipherSuiteProvider::hpke_seal_psk).
    /// The same [`HpkePsk`] used during encryption must be provided.
